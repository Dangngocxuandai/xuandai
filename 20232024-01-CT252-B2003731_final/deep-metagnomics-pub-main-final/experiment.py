...
def para_cmd ():
    ...
    ## dnxdai
    parser.add_option('--umap_metric',help='umap hyperparameter: metric (euclidean/manhattan/chebyshev/minkowski/seuclidean/cosine/canberra/braycurtis/haversine/mahalanobis/wminkowski/seuclidean/correlation/hamming/jaccard/dice/russellrao/kulsinski/rogerstanimoto/sokalmichener/sokalsneath/yule)')
    parser.add_option('--umap_min_distance',help='umap hyperparameter: umap_min_distance (float)')
    parser.add_option('--kernel_size',help='MobileNetV1 hyperparameter: kernel_size (int)')
    parser.add_option('--stride',help='MobileNetV1 hyperparameter: stride (int)')
    parser.add_option('--alp',help='MobileNetV1 hyperparameter: alp (int)')
    parser.add_option('--ro',help='MobileNetV1 hyperparameter: ro (int)')
    parser.add_option('--dropout_perc',help='MobileNetV1 hyperparameter: dropout_perc (float)')
    parser.add_option('--filters',help='MobileNetV1 hyperparameter: filters (float)')
...
def get_default_value (options):
    ...
    if not options.umap_metric:  #if did not set from cmd, use default value
        options.umap_metric = 'wminkowski'

    if not options.umap_min_distance:  #if did not set from cmd, use default value
        options.umap_min_distance = 0.1
    if not options.kernel_size:  #if did not set from cmd, use default value
        options.kernel_size = 3
    if not options.stride:  #if did not set from cmd, use default value
        options.stride = 2
    if not options.alp:  #if did not set from cmd, use default value
        options.alp = 1
    if not options.ro:  #if did not set from cmd, use default value
        options.ro = 1
    if not options.dropout_perc:  #if did not set from cmd, use default value
        options.dropout_perc = 0.1
    if not options.filters:  #if did not set from cmd, use default value
        options.filters = 0
    # data_type_qrcode
...
def para_config_file (options):
    ...
    if not options.umap_metric:  #if did not set from cmd
        if config.has_option('Visual','umap_metric'): #if avaiable from config file
            options.umap_metric = config.get('Visual','umap_metric')
        else: #other cases, use default value
            options.umap_metric = 'euclidean'
            
    if not options.umap_min_distance:  #if did not set from cmd
        if config.has_option('Visual','umap_min_distance'): #if avaiable from config file
            options.umap_min_distance = config.get('Visual','umap_min_distance')
        else: #other cases, use default value
            options.umap_min_distance = 0.1
    if not options.dropout_perc:  #if did not set from cmd
        if config.has_option('Visual','dropout_perc'): #if avaiable from config file
            options.dropout_perc = config.get('Visual','dropout_perc')
        else: #other cases, use default value
            options.dropout_perc = 0.1
...
def string_to_numeric(options):
    ...
    options.umap_min_distance =  float(options.umap_min_distance)
...
def write_para(options,configfile):
    ...
    Config.set('Visual','umap_metric',options.umap_metric )
    Config.set('Visual','umap_min_distance',options.umap_min_distance )
...
def validation_para(options):
    ...
    if not (options.type_emb in ['raw','bin','fill','fills','zfill','zfills','tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom','none','qrcode','umap']):
        print  ('you entered --type_emb=' + str(options.type_emb))
        print  (utils.textcolor_display('--type_emb') + " only supports 'raw','bin','fill','fills','zfill','zfills','tsne','umap','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom, 'qrcode")
        valid = 0
    ...
    if not (options.model in ['svm_model','rf_model','dtc_model','gbc_model','knn_model','model_cnn','pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d','MobileNetV1']):
        print  ('you entered --model=' + str(options.model))
        print  (utils.textcolor_display('--model') + " only supports 'svm_model','rf_model','dtc_model','gbc_model','knn_model','model_cnn','pretrained','fc_model','model_lstm','model_mlp','model_vgglike','model_cnn1d','MobileNetV1")
        valid = 0
...
def naming_folder (options, search_res = 'y'):
    ...
    if options.type_emb == 'raw':      #raw data  
        ...
        if options.type_emb == 'umap':
            pre_name = pre_name + '_metric_' + str(options.umap_metric)  + '_mdis_' + str(options.umap_min_distance) + '_mdis_' + '_p'+str(perplexity_x)
...
def visual_section (options,  train_x,val_x, train_y, val_y, v_data, v_labels,train_x_original,
    ...
    if options.type_emb != 'none': # added by tbtoan   
        ...
        # generate bin breaks
                        if options.type_bin == 'GMM':
                            bins_breaks = vis_data.build_breaks_gmm (data=train_x, type_bin=options.type_bin, num_bin=options.num_bin, 
                                max_v= max_v_temp, min_v = min_v_temp,
                                gmm_init_params = options.gmm_init_params, gmm_covariance_type =  options.gmm_covariance_type )
                                #gmm_init_params = 'kmeans', gmm_covariance_type = 'full')
                        else:
        ...
        if options.type_emb in ['fills','zfills'] and options.type_bin != 'pr': #fills: use fill-up with bins created from train-set with scale, should be used with type_bin='eqw'
        ...
        elif options.type_emb in  ['tsne','megaman','lle','isomap','mds','se','pca','rd_pro','lda','nmf','minisom','gsom','qrcode','umap'] : #if use other embeddings (not fill), then now embedding!
            ...
            if not os.path.exists(path_write+"/val_" + str(len(val_y)-1) + ".png") or options.recreate_img > 0 : 
                ...
                if options.type_emb =='umap': 
                    ...
                    if options.label_visual != "": # Nếu có nhãn cụ thể được chỉ định
                        if int(options.label_visual) == 1 or int(options.label_visual) ==0:
                            input_x_filter = []

                            train_y1 = train_y.reset_index().drop('index', 1)

                            for i_umap in range(len(train_y1)):
                                if train_y1.x[i_umap] == int(options.label_visual):
                                    input_x_filter.append(train_x[i_umap])
                            
                            input_x_filter = np.transpose(input_x_filter)

                            # Sử dụng UMAP để giảm chiều dữ liệu
                            X_embedded = umap.UMAP(n_neighbors=options.perlexity_neighbor, min_dist=options.umap_min_distance, n_components=options.n_components_emb, metric=str(options.umap_metric)).fit_transform(input_x)
                            # X_embedded = umap.UMAP(n_neighbors=options.perlexity_neighbor, min_dist=options.umap_min_distance, n_components=options.n_components_emb).fit_transform(input_x)

                    else: # Nếu không có nhãn cụ thể được chỉ định
                        # Sử dụng UMAP để giảm chiều dữ liệu
                        X_embedded = umap.UMAP(n_neighbors=options.perlexity_neighbor, min_dist=options.umap_min_distance, n_components=options.n_components_emb, metric=str(options.umap_metric)).fit_transform(input_x)
                        # X_embedded = umap.UMAP(n_neighbors=options.perlexity_neighbor, min_dist=options.umap_min_distance, n_components=options.n_components_emb).fit_transform(input_x)
        ...
        #     bins_breaks = vis_data.build_breaks (data=data, type_bin=options.type_bin, num_bin=options.num_bin, max_v= max_v_temp, min_v = min_v_temp)
            ...
            if options.type_run  in ['vis','visual']:  
                ...
                model = models_def.call_model (type_model=options.model, 
                        ...
                        kernel_size=options.kernel_size,stride=options.stride,alp=options.alp,ro=options.ro,dropout_perc=options.dropout_perc,filters=options.filters
                ...
                if options.model in ['svm_model','dtc_model', 'rf_model','gbc_model','knn_model'] or (options.model in [ 'pretrained'] and options.pretrained_w_path[len(options.pretrained_w_path)-2]+options.pretrained_w_path[len(options.pretrained_w_path)-1] in ['ml']):
                    ...
                    history_callback=model.fit(train_x, train_y, 
                        ...
                        shuffle=False)     
                    # history_callback=model.fit_generator(generator=train_x,
                    #                 epochs=options.epoch, validation_data=(val_x, val_y),
                    #                callbacks=[early_stopping])
                    #     shuffle=False)
                    # if shuffle=False could be reproducibility
