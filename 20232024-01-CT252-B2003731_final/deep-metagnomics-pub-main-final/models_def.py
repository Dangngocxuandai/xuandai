...
from keras.layers import DepthwiseConv2D, BatchNormalization, AveragePooling2D, Reshape, Add, Dropout
...
def depthwise_block(inputs,stride,kernel_size=3,pad='same'):
    '''
        Function that performs the depthwise convolution
        Inputs:
            inputs:    the input shape of the depthwise convolution
            kernel_size: a tuple that indicates the size of the filtering kernel
            stride:    a tuple that indicates the stride of the kernel
        Return:
            x: the result of the depthwise convolution
    '''
        
    x = DepthwiseConv2D(kernel_size=kernel_size,strides=stride,depth_multiplier=1,padding=pad)(inputs)
    x = BatchNormalization()(x)
    x = Activation(activation='relu')(x)
        
    return x

def pointwise_block(inputs,num_filters,alpha,kernel_size=1,strides=1,pad='same',dropout=False,dropout_perc=0.1):
    '''
    ...
    x = Conv2D(filters=n_fil,kernel_size=kernel_size,padding=pad)(inputs)
    x = BatchNormalization()(x)
    x = Activation(activation='relu')(x)
...
def call_model (type_model,m_input_shape, m_num_classes, m_optimizer,m_learning_rate, m_learning_decay,m_loss_func,
            ...
            svm_c, svm_kernel, rf_n_estimators,rf_max_depth, m_pretrained_file,knn_n_neighbors, rf_max_features,kernel_size,stride,alp,ro,dropout_perc,filters,m_type_emb=''):
    ...
    elif type_model== "MobileNetV1":
        model=MobileNetV1(m_input_shape,m_num_classes,filters=filters,kernel_size=kernel_size,stride=stride,alp=alp,ro=ro,dropout_perc=dropout_perc)
        # model = MobileNetV1(m_input_shape,
        #                 alpha=alpha,
    ...
    return x
def MobileNetV1(input_shape,num_units,filters=32,kernel_size=3,stride=2,alp=1,ro=1,dropout_perc=0.1):
    input_shape = (int(input_shape[0] * ro), int(input_shape[1] * ro), input_shape[2])
    
    inputs = Input(shape=input_shape)
    
    # Regular convolution
    x = Conv2D(filters=filters,kernel_size=kernel_size,strides=stride)(inputs)
    x = BatchNormalization()(x)
    x = Activation(activation='relu')(x)
    x = Dropout(dropout_perc)(x)
    
    # First depthwise-pointwise block
    x = depthwise_block(x,kernel_size=3,stride=1)
    x = pointwise_block(x,num_filters=64,alpha=alp,strides=1,dropout=True,dropout_perc=dropout_perc)
    
    # Second depthwise-pointwise block
    x = depthwise_block(x,kernel_size=3,stride=2)
    x = pointwise_block(x,num_filters=128,alpha=alp,stride=1,dropout=True,dropout_perc=dropout_perc)
    
    # Third depthwise-point .wise block
    x = depthwise_block(x,kernel_size=3,stride=1)
    x = pointwise_block(x,num_filters=128,alpha=alp,stride=1,dropout=True,dropout_perc=dropout_perc)
    
    # Fourth depthwise-pointwise block
    x = depthwise_block(x,kernel_size=3,stride=2)
    x = pointwise_block(x,num_filters=256,alpha=alp,stride=1,dropout=True,dropout_perc=dropout_perc)    
    ...
    x = depthwise_block(x,kernel_size=3,stride=2)
    x = pointwise_block(x,num_filters=512,alpha=alp,stride=1,dropout=True,dropout_perc=dropout_perc)
    ...
    x = depthwise_block(x,kernel_size=3,stride=2)
    x = pointwise_block(x,num_filters=1024,alpha=alp,stride=1,dropout=True,dropout_perc=dropout_perc)
    ...
    x = AveragePooling2D(padding='valid',pool_size=(pool_size,pool_size),stride=1)(x)    
